...
#    TEACHER_SIGNUP = USERS_URL
    ...
    def login(cls, token, credentials, *args, **kwargs):
        ...
        if 'X-Cookie-Peek' not in csrf_response.headers:
            ...
            cls.debug(cls.route_url(cls.CSRF_TOKEN_PAGE)),
            cls.debug_http_package(csrf_response.request, label='CSRF request',
                    secret_body_values=secret_body_values)
            cls.debug_http_package(csrf_response, label='CSRF response')
            return False
        else:
